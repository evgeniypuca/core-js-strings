...
function countVowels(str) {
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if ('AEIOUY'.includes(str[i].toUpperCase())) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  const newReversedStr = newStr.split('').reverse().join('');
  return newStr === newReversedStr;
}
...
function findLongestWord(sentence) {
  const sortArr = sentence.split(' ').sort((a, b) => b.length - a.length);
  return sortArr[0];
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    result += rot13Letters.charAt(index);
  }
  return result;
}
